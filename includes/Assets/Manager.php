...
class Manager
{
    public function register()
    {
        ...
        add_action('wp_enqueue_scripts', [$this, 'load_wc_blocks'], 5);
        add_action('admin_enqueue_scripts', [$this, 'load_wc_blocks'], 5);
    }
    
    /**
     * load WooCommerce Blocks
     */
    public function load_wc_blocks(): void
    {
        // Check if WooCommerce Blocks is active
        if (!class_exists('Automattic\WooCommerce\Blocks\Package')) {
            return;
        }
        // // If available, use WooCommerce Blocks Package class
        // if (class_exists('Automattic\WooCommerce\Blocks\Assets')) {
        //     $assets = new \Automattic\WooCommerce\Blocks\Assets();
        //     if (method_exists($assets, 'register_script')) {
        //         // WooCommerce Blocks will auto-enqueue on pages with blocks
        //         // We just need to ensure they're registered
        //     }
        // }
        
        // Directly enqueue the scripts
        if (wp_script_is('wc-blocks', 'registered')) {
            wp_enqueue_script('wc-blocks-vendors');
            wp_enqueue_script('wc-blocks-data-store');
            wp_enqueue_script('wc-blocks');
  ...
  public function register_block_assets()
{
    ...
    foreach (glob($build_path . '*/') as $block_path) {
        ...
        wp_register_script(
            $handle,
            MH_ACADEMY_PLUGIN_ROOT_DIR . "build/{$block_slug}/view.js",
            $asset['dependencies'],
            $asset['version'],
            true
        );
